...
char M[100][4];//Memory
char BUFF[40];//Buffer
int SI=0;//System Interrupt
fstream input;//stream for taking input from input file
fstream output;//stream for printing output in output file
void clearbuff()
{
for(auto i=0;i<40;i++)
{
...
init();
int i=0;
label://indicate the starting address of a function or subroutine, or to provide a target for a jump or branch instruction.
getline(input,line);
for(auto i=0;i<line.size() && i<40;i++)
{
BUFF[i]=line[i];
...
while(BUFF[k]!='-' && k!=40)
{
M[word_no][j]=BUFF[k];//to differentiate between the H instruction and 'H' string
if(BUFF[k]=='H'&& SI!=1)
{
j+=3;
}
